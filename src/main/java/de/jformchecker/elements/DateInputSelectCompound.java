...
public class DateInputSelectCompound extends AbstractInput<DateInputSelectCompound>
  ...
  public String getInputTag(Map<String, String> attributes) {
    ...
    return day.getInputTag(tagAttributes.getAttributes())
        + month.getInputTag(tagAttributes.getAttributes())
  ...
  public void init(Request request, boolean firstRun, Validator validator) {
    ...
    if (firstRun) {
      ...
      if (!isRequired() && StringUtils.isEmpty(day.getValue())
          && StringUtils.isEmpty(month.getValue()) && StringUtils.isEmpty(year.getValue())) {
        this.valid = true;
        ...
        try {
          internalDate = LocalDate.of(Integer.parseInt(year.getValue()),
              Integer.parseInt(month.getValue()), Integer.parseInt(day.getValue()));
          this.valid = true;
  ...
  /**
   * a range of years - that can be shown in the year-dropdown Use the static convenience methods
   * 
   * @author jochen
   *
   */
  public static class YearRange {
    ...
    /**
     * year around now. Example: AroundNow(2) will return for the currentYear=2017 :
    ...
    /**
     * year around year. Example: AroundNow(2019, 2) will return : 2017,2018,2019,2020,2021
     * 
     * @param offset
     * @return
     */
    public static YearRange aroundYear(int year, int offset) {
      return new YearRange(year - offset, year + offset);
    }
